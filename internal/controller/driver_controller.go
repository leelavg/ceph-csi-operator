...
func (r *DriverReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	enqueueAllDrivers := handler.EnqueueRequestsFromMapFunc(
		func(_ context.Context, obj client.Object) []reconcile.Request {
			...
			if err := r.List(context.Background(), &driverList); err != nil {
...
func (r *driverReconcile) LoadAndValidateDesiredState() error {
	...
	if err := r.Get(r.ctx, client.ObjectKey{Name: r.driver.Name}, &storagev1.CSIDriver{}); err == nil {
...
func (r *driverReconcile) reconcileControllerPluginDeployment() error {
	...
	_, err := ctrlutil.CreateOrUpdate(r.ctx, r.Client, deploy, func() error {
		if err := ctrlutil.SetOwnerReference(&r.driver, deploy, r.Scheme); err != nil {
			log.Error(err, "Faild setting an owner reference on deployment")
			return err
		}
		...
		provisionerSpec := utils.FirstNonNil(r.driver.Spec.Provisioner, &csiv1a1.ProvisionerSpec{})
		imagePullPolicy := utils.FirstNonEmpty(provisionerSpec.ImagePullPolicy, corev1.PullIfNotPresent)
		...
		deploy.Spec = appsv1.DeploymentSpec{
			Replicas: provisionerSpec.Replicas,
			...
			Template: corev1.PodTemplateSpec{
				ObjectMeta: metav1.ObjectMeta{
					Labels: (func() map[string]string {
						...
						maps.Copy(podLabels, provisionerSpec.Labels)
						podLabels["app"] = appName
						return podLabels
					})(),
					Annotations: maps.Clone(provisionerSpec.Annotations),
				},
				Spec: corev1.PodSpec{
					PriorityClassName: ptr.Deref(provisionerSpec.PrioritylClassName, ""),
					Affinity:          provisionerSpec.Affinity,
					Tolerations:       provisionerSpec.Tolerations,
					Containers: (func() []corev1.Container {
						containers := []corev1.Container{
							// Plugin Container
							{
								...
								VolumeMounts: (func() []corev1.VolumeMount {
									mounts := append(
										// Add user defiend volume mounts at the start to make sure they do not
										// overwrite built in volumes mounts.
										utils.MapSlice(
											provisionerSpec.Volumes,
											func(v csiv1a1.VolumeSepc) corev1.VolumeMount {
												return v.Mount
											},
										),
								...
								Resources: ptr.Deref(
									provisionerSpec.Resources.Plugin,
							...
							// Provisioner Sidecar Container
							{
								...
								Resources: ptr.Deref(
									provisionerSpec.Resources.Provisioner,
									corev1.ResourceRequirements{},
								),
							},
							// Resizer Sidecar Container
							{
								...
								Resources: ptr.Deref(
									provisionerSpec.Resources.Resizer,
									corev1.ResourceRequirements{},
								),
							},
							// Attacher Sidecar Container
							{
								...
								Resources: ptr.Deref(
									provisionerSpec.Resources.Attacher,
									corev1.ResourceRequirements{},
								),
							},
							// Snapshotter Sidecar Container
							{
								...
								Resources: ptr.Deref(
									provisionerSpec.Resources.Snapshotter,
									corev1.ResourceRequirements{},
								),
							},
						}
						...
						if ptr.Deref(r.driver.Spec.DeployCsiAddons, false) {
							containers = append(containers, corev1.Container{
								...
								Resources: ptr.Deref(
									provisionerSpec.Resources.Addons,
									corev1.ResourceRequirements{},
								),
							})
						}
						...
						if r.isRdbDriver() && ptr.Deref(r.driver.Spec.GenerateOMapInfo, false) {
							containers = append(containers, corev1.Container{
								...
								Resources: ptr.Deref(
									provisionerSpec.Resources.OMapGenerator,
									corev1.ResourceRequirements{},
								),
							})
						}
						...
						if r.driver.Spec.Liveness != nil {
							containers = append(containers, corev1.Container{
								...
								Resources: ptr.Deref(
									provisionerSpec.Resources.Liveness,
									corev1.ResourceRequirements{},
								),
							})
						}
					...
					Volumes: (func() []corev1.Volume {
						volumes := append(
							// Add user defiend volumes at the start to make sure they do not
							// overwrite built in volumes.
							utils.MapSlice(
								provisionerSpec.Volumes,
								func(v csiv1a1.VolumeSepc) corev1.Volume {
									return v.Volume
								},
							),
							utils.HostDevVolume,
...
func (r *driverReconcile) reconcileNodePluginDeamonSet() error {
	...
	_, err := ctrlutil.CreateOrUpdate(r.ctx, r.Client, daemonSet, func() error {
		...
		pluginSpec := utils.FirstNonNil(r.driver.Spec.Plugin, &csiv1a1.PluginSpec{})
